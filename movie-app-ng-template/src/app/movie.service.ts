...
export class MovieService {
  ...
  private _movieUrl ='http://localhost:8080/movie/api/v1/';
  ...
  getFavourites():any{
    let search_url = `${this._movieUrl}movies`;
    return this.http.get(search_url);
  }
  addFavourites(newFav: favourites):any{
    let post_url = `${this._movieUrl}movies`;
    return this.http.post(post_url,newFav,httpOptions);
  }
  checkIfFav(id):any{
    let _url = `${this._movieUrl}movies/${id}`;
  ...
  updateComments(favObj):any{
    let _url = `${this._movieUrl}movies`;
